...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and tnechain address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an tnechain transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "tnechain app in browser mode", w.failure
	}
	if w.offline() {
		return "tnechain app offline", w.failure
	}
	return fmt.Sprintf("tnechain app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whtster the wallet and the tnechain app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// tnechain app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the tnechain app's version, will fail prior to v1.0.2
...
// and returning the tnechain address located on that derivation path.
...
// ledgerVersion retrieves the current version of the tnechain wallet app running
...
// ledgerDerive retrieves the currently active tnechain address from a Ledger
...
//   tnechain address length | 1 byte
//   tnechain address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the tnechain hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an tnechain address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the tnechain signature and do a sanity validation
	if len(reply) != 65 {
