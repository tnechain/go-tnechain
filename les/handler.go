...
// NewProtocolManager returns a new tstereum sub protocol manager. The tnechain sub protocol manages peers capable
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light tnechain protocol")
	...
	log.Info("Light tnechain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light tnechain peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light tnechain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light tnechain peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light tnechain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light tnechain message arrived", "code", msg.Code, "bytes", msg.Size)
...
// NodeInfo represents a short summary of the tnechain sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // tnechain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
