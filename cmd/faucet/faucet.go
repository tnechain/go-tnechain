...
import (
	...
	"github.com/tnechain/go-tnechain/accounts/keystore"
	...
	"github.com/tnechain/go-tnechain/p2p/discover"
	"github.com/tnechain/go-tnechain/p2p/discv5"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the tnechain protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for tnechain and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // tnechain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an tnechain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // tnechain protocol stack
	client *tstclient.Client   // Client connection to the tnechain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the tnechain light client protocol
...
// close terminates the tnechain connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the tnechain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somtsting funky happened, please open an issue at https://github.com/tnechain/go-tnechain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and tnechain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for tnechain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No tnechain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the tnechain address and profile URL.
...
// returning the username, avatar URL and tnechain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain tnechain address,
